...
@Component({
  selector: 'app-lr-6',
  templateUrl: './lr-6.component.html',
  styleUrls: ['./lr-6.component.css']
})
export class Lr6Component implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.cookieValue != '' && this.cookieValue != undefined){
      this.cookieValue = JSON.parse(this.cookieService.get('signupData'));
  ...
  onSubmit(){
    ...
    // this.cookieValue = this.cookieService.get('loginData');
     ...
     this.transporter_id=this.cookieValue.transporter_id
    this.apiSerivce.addtemplate({'transporter_id':this.transporter_id,'template_id':'6','template_color':this.schemecolor,'template_type': '6'}).subscribe( 
